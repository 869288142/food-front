<template>
  <div class="r">
    <ul class="r-list">
      <li v-for="(restaurant, index) in restaurants" :key="index" class="r-list-item">
        ...
        <div class="r-list-item-wrap">
          ...
          <span class="modify" @click="modify(restaurant.id)">修改</span>
          <span>{{restaurant.city.name}}</span>
          <span class="delete" @click="remove(restaurant.id)">删除</span>
        </div>
      </li>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    ...
    async remove(restaurant_id) {
      ...
      this.restaurants = await this.apiGet("/restaurant", { id: this.user_id })
    },
